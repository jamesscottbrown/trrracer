...
export default function groupBubbles(groupBy, wrap, forced, selectedActivityURL, filteredActivities, setTool, setHoverData, researchThreads) {
    ...
    if(linkDataAfter.length > 0){
      ...
      midG.filter(f => f.id === gData.id).append('path')
  ...
  activityGroups.on('mouseover', (event, d)=> {
    console.log(event, d)
    d3.selectAll('.activity').filter(f => f.activity_uid === d.activity_uid).attr('stroke', 'red').attr('stroke-width', '2px')
    d3.selectAll('circle.hidden').filter(f => {
      if(f){
