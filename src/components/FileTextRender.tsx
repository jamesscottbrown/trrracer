...
const FileTextRender = (fileDataProps:any) => {
  ...
  return (
      ...
      {fileData.fileType === "gdoc" ? (<Box>
        {
          fileData.emphasized ? <Box>{fileData.emphasized.map((em:any, i:number) => (
        ...
        (<Box>
          {   keywordArray.filter((k:any) => k['file-title'] === fileData.title)[0]  ? 
              keywordArray.filter((k:any) => k['file-title'] === fileData.title)[0].keywords.keywords.map((m:any) => (
